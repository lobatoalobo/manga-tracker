...
import type { PreorderClose, PreorderListRow } from "./mock-preventas";
...
function CloseCol({ close }: { close: PreorderClose }) {
  const muted = close.primary === "Sin fecha";
  return (
    <div className="leading-tight lg:w-28 lg:shrink-0">
      {close.label ? <div className="text-xs text-slate-400">{close.label}</div> : null}
      <div className={`text-sm font-semibold ${muted ? "text-slate-400" : "text-slate-900"}`}>{close.primary}</div>
...
/** Una preventa como tarjeta horizontal amplia (una fila en desktop; se apila en mobile). Solo presentación. */
export function PreorderListItem({ row }: { row: PreorderListRow }) {
  return (
    <article className="rounded-2xl border border-slate-200/70 bg-white p-4 shadow-sm">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:gap-5">
        {/* Identidad: barra de estado + collage + título */}
        <div className="flex items-center gap-4 lg:min-w-0 lg:flex-1">
          <span className={`h-14 w-1.5 shrink-0 rounded-full ${row.bar}`} aria-hidden />
          <Collage n={row.covers} />
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <h3 className="truncate font-semibold text-slate-900">{row.title}</h3>
              <PreorderStatusBadge estado={row.estado} />
            </div>
            <p className="mt-0.5 truncate text-sm text-slate-500">{row.publishers}</p>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-6 lg:gap-7">
          ...
          <div className="grid grid-cols-3 gap-4 sm:flex sm:gap-6 lg:gap-7">
            ...
            <Metric label="Reservado" value={row.reservado} className="lg:w-28" />
          </div>
          <div className="flex items-center gap-2 sm:ml-auto lg:ml-0">
            <button type="button" className="flex-1 rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-violet-600 transition-colors hover:bg-violet-50 sm:flex-none">
              {row.cta}
            </button>
