...
| L5 | 🟠 | ✅ | **BUG real encontrado en prod**: 103/132 works con `upcoming=true` ya tenían edición publicada (Berserk, One-Punch Man, etc. salían como "🔜 Próximo a salir"). Causa: `upcoming` es un flag cacheado que el crawl de Whakoom dejaba viejo (sumaba tomos) hasta el reconcile diario. Fix: (1) corrección de datos en prod (103→0 contradictorias), (2) **guard en read-time**: nunca mostrar próximo si la obra tiene edición con `volumes>0` — en `browseWorks` (tab series + badge), `upcomingForIds`, y ficha `/serie`. Bulletproof ante flag viejo o cron caído. | `lib/catalog.ts`, `app/(browse)/serie/[id]` |
