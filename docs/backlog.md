...
- **Retail / Collection (Slice 8) — diferidos.** La colección automática por `PICKED_UP` está ✅ implementada (bounded context Collection: `Acquisition`+`OwnershipPosition`; `docs/retail-slice-8-collection-projection.md`, ADR-010). Diferido: **read-side unificado y migración desde `OwnedVolume`/`Purchase`** (ejes de identidad incompatibles) → una slice posterior; **reversa de pickup** (concepto `Disposal`); ejemplares individualizados / pool fungible; multi-canal; watermark persistido / tabla de jobs del barrido; **promoción de la migración `20260801000000_slice8_collection` a producción** (gated, hoy solo efímero/staging = checkpoint de deploy).
