...
**Dónde se usa** — Todas.
**Props** — `label` · `variant` (primary | ghost | warn) · `size` (default | small) · `disabled` · `onClick`.
...
**Dónde se usa** — Todas.
**Props** — `serie` · `volumen` · `imagen?` (futuro; hoy greybox por paleta) · `estadoVisual` · `size` (xs…xl).
...
**Props** — `label` · `tono` (neutral | mark | warn | go) · `dot?` (bool) · `prefijo?` (p. ej. "+") · `onClick?`.
...
**Props** — `tomo` ({serie, volumen, color/imagen, autor?}) · `cantidad?` · `precio?` · `mostrarPrecio` · `marca?` (faltante | atenuada) · **`accion` (slot)** — la pantalla inyecta el control.
...
**Props** — `principal` (tomo | null) · `secundarias` (tomo[]) · `modo` (editor | publica) · **`accionesTomo` (slot)** en modo editor (llevar/bajar/principal).
...
**Props** — `modo` (adjuntar | enviar | ver | confirmar) · `archivo?` · `onAdjuntar?/onEnviar?/onConfirmar?`.
...
**Props** — `edicion` ({número, semana, estado}) · `faseActual` (creación | preventa | cierre | preparación | entrega) · `children`.
...
**Props** — `status` (texto | bloqueo con motivo) · `botones` (C-01[]) · `sticky`.
...
**Props** — `abierta` · `onCerrar` · `children`.
...
**Dónde se usa** — P-08.
**Props** — `placeholder` · `valor` · `onChange` · `resultados`.
