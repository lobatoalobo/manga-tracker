...
describe("infra write-port — persistencia, gate, dedup, atomicidad", () => {
  it("lock de la propuesta primero; crea Work (upcoming) y update único del ResolutionRecord", async () => {
    ...
    expect(out).toEqual({ proposalId: 5, resolutionRecordId: 42, appliedWorkId: 777, mutationCorrelationId: "corr-9", recovered: false });
  ...
  it("propuesta inexistente / targetKind no soportado / estado no ACEPTADA", async () => {
    ...
    await expect(runApply(fakeTx({ locked: [{ id: 5, status: "ACEPTADA", targetKind: "NEW_EDITION", contentClass: "MANGA", version: 2 }] }))).rejects.toThrow(TargetKindNotSupportedError);
  ...
  it("replay consistente: recupera, NO crea Work ni actualiza ResolutionRecord", async () => {
    ...
    expect(out).toEqual({ proposalId: 5, resolutionRecordId: 42, appliedWorkId: 555, mutationCorrelationId: "corr-old", recovered: true });
    expect(tx.work.create).not.toHaveBeenCalled();
...
describe("mutación applyCatalogProposal — audit", () => {
  ...
  it("kind CONTRIB_APPLY_PROPOSAL, correlationId propagado al write-port, affected creation, sin contenido", async () => {
    const apply = vi.fn().mockResolvedValue({ proposalId: 5, resolutionRecordId: 42, appliedWorkId: 777, mutationCorrelationId: "corr-XYZ", recovered: false });
  ...
  it("recovered → affected en cero", async () => {
    const write: ApplyWritePort = { apply: vi.fn().mockResolvedValue({ proposalId: 5, resolutionRecordId: 42, appliedWorkId: 555, mutationCorrelationId: "c", recovered: true }) };
    const tx: TransactionRunner<ApplyReadPort, ApplyWritePort> = { run: (fn) => fn({ read, write }) };
